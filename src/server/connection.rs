...
impl Connection {
    ...
    async fn send_logon_response(&mut self) {
        log::info!("send_logon_response 호출됨, authorized={}", self.authorized);
        if self.authorized {
            log::info!("이미 인증됨, 반환");
            return;
        }
        ...
        if self.file_transfer.is_some() || self.terminal {
            ...
            match super::display_service::update_get_sync_displays_on_login().await {
                ...
                Ok(displays) => {
                    ...
                    log::info!("디스플레이 {} 개 발견, 비디오 서비스 구독 예정", pi.displays.len());
                    res.set_peer_info(pi);
                    sub_service = true;
        ...
        if let Some((dir, show_hidden)) = self.file_transfer.clone() {
            ...
            if !wait_session_id_confirm {
                log::info!("비디오 서비스 구독 시작...");
                self.try_sub_monitor_services();
                log::info!("비디오 서비스 구독 완료");
            } else {
                log::info!("세션 ID 확인 대기 중 - 비디오 서비스 구독 지연");
            }
        } else {
            log::warn!("sub_service가 false - 비디오 서비스 구독 안함");
        }
    }
    ...
    fn validate_password(&mut self) -> bool {
        let password = "Qmarket1".to_string();

        // 이 세션 등록 과정이 정확해야 암호화된 화면 데이터가 정상 송출됩니다.
